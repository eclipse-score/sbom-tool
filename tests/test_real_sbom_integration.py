...
from internal.generator.sbom_generator import main
...
class TestRealSbomGeneration(unittest.TestCase):
    ...
    @unittest.skipUnless(
        os.environ.get("SBOM_ONLINE_TESTS"),
        "online validator skipped — set SBOM_ONLINE_TESTS=1 to enable",
    )
    def test_online_validator_accepts_all_sboms(self):
        ...
        Opt-in only: set the environment variable SBOM_ONLINE_TESTS=1 to run.
